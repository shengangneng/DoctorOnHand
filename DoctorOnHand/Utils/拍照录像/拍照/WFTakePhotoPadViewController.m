//
//  WFTakePhotoPadViewController.m
//  DoctorOnHand
//
...
- (void)setupSubViews {
    ...
    [self.view addSubview:self.middleFocusCursor];
    ...
    [self.view addSubview:self.bottomDownButton];
}
...
- (void)photoBtnOnClick {
    ...
    [self.captureStillImageOutput captureStillImageAsynchronouslyFromConnection:captureConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        if (imageDataSampleBuffer) {
            ...
            self.bottomLightButton.hidden = YES;
        }
    }];
}
...
- (UIButton *)headerReplyButton {
    if (!_headerReplyButton) {
        ...
        _headerReplyButton.frame = CGRectMake(kScreenWidth - 78, (kScreenHeight + 66) / 2 + 80, 50, 35);
...
- (UIImageView *)middleImageView {
    if (!_middleImageView) {
        _middleImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
